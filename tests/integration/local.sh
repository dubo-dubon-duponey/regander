...
# XXX Hub metadata update is asynchronous, meaning some of these will fail the first time
...
dc-tools::assert::equal "$exit" "0" "catalog GET"
dc-tools::assert::equal "$result" '{"repositories":[]}'
...
dc-tools::assert::equal "$exit" "0" "catalog GET"
dc-tools::assert::equal "$result" "{\"repositories\":[\"$otherimagename\",\"$imagename\"]}"
